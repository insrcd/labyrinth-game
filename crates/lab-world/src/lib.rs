...
impl Plugin for WorldPlugin {
    fn build(&self, app: &mut AppBuilder) {
        app.add_resource(InteractionCatalog::<
            TileInteraction,
            TileComponents,
            Vec<TileInteractionResult>,
        >::default())
            ...
            .add_system_to_stage(
                lab_core::stages::POST_UPDATE,
                systems::interaction_system.system(),
            )
            .add_system_to_stage(
                lab_core::stages::POST_UPDATE,
                systems::process_interaction_result_system.system(),
            )
            .add_system(systems::add_text_to_adventure_log.system())
            .add_system_to_stage(
                lab_core::stages::POST_UPDATE,
                systems::static_text_system.system(),
            )
...
pub struct TileInteraction {
    ...
    pub caller: fn(
        InteractionContext<TileInteraction, TileComponents, Vec<TileInteractionResult>>,
    ) -> Vec<TileInteractionResult>,
}
...
impl Interact<TileComponents, Vec<TileInteractionResult>> for TileInteraction {
    fn interact(
        &self,
        ctx: InteractionContext<Self, TileComponents, Vec<TileInteractionResult>>,
    ) -> Vec<TileInteractionResult> {
        (self.caller)(ctx)
    }
}
