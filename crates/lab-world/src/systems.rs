...
pub fn tile_interaction_system (
    ...
    for (tile_entity, _tt,  
        ...
        for ( _m, mut move_transition, movement, move_sprite,scale) in &mut moveables.iter() {
            println!("Checking for collision");

            let collision = collide(move_transition.0, 
                (move_sprite.size() * scale.0) - Vec2::new(16.* scale.0,16.* scale.0),  tile_translation.0, tile_sprite.size() * scale.0, false);
