...
impl Plugin for SpritesPlugin {
    fn build(&self, app: &mut AppBuilder) {
        app.add_resource(SpriteLibrary::new())
            .add_startup_system_to_stage(
                stages::INIT,
                crate::systems::load_world_sprites_system.system(),
            );
    }
}
...
impl SpriteLibrary {
    ...
    pub fn catalog_sprites(
        ...
        let texture_handle = asset_server.load_sync(assets, filename).unwrap();
    ...
    pub fn sprites_in_category(&self, category: &str) -> Vec<SpriteInfo> {
        let mut sorted_vec: Vec<SpriteInfo> = self
            .library
            .values()
    ...
    pub fn write_text(&self, commands: &mut Commands, st: String, location: Vec3) -> Vec<Entity> {
        self.make_string(st, location)
            .into_iter()
            .map(move |c| {
                commands
                    .spawn(c.1.to_components(c.0, 1.))
    ...
    pub fn place_despawning_sprite(
        &self,
        commands: &mut Commands,
        name: String,
        scale: f32,
        ...
        commands
            .spawn(self.get(&name).unwrap().to_components(location, scale))
