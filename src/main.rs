...
fn setup(
    ...
    let font_handle = asset_server
        .load_sync(&mut assets, "resources/fonts/FiraSans-Bold.ttf")
        .unwrap();

    log.make(&mut commands, font_handle, 4)
    ...
    text_change.send(TextChangeEvent {
        name: "".to_string(),
        text: "Get Adventuring!".to_string(),
    });

    commands
        .spawn(UiCameraComponents::default())
        .spawn(Camera2dComponents::default())
        .spawn((state::SceneState {
            next_state: state::StateType::Init,
        },))
        .spawn((Mouse::default(),))
